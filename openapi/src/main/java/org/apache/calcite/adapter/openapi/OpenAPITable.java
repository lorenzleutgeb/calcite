...
public class OpenAPITable extends AbstractTable implements FilterableTable {
    ...
    public Enumerable<Object[]> scan(DataContext root, List<RexNode> filters) {
        ...
        if (firstFilter.isA(SqlKind.EQUALS))
            ...
            count = ((RexCall) firstFilter).operands.stream().filter(filter -> findSourcePath(filter)).count();
            ok = count == 1;
        } else
            throw new RuntimeException("Where clause must be a single constraint or an AND.");
        if (!ok)
            throw new RuntimeException(String.format("Need exactly one filter that maps to an API path (have %d).", count));
        assert (sourceURL != null);
        final int[] fields = IntStream.range(0, order.size()).toArray();
        final AtomicBoolean cancelFlag = DataContext.Variable.CANCEL_FLAG.get(root);
        return new AbstractEnumerable<Object[]>() {
            public Enumerator<Object[]> enumerator() {
                return new OpenAPIEnumerator<>(sourceURL, cancelFlag, entityModel, order, fields, sourcePath);
            }
        };
    }
...
//            throw new UnsupportedOperationException("Invalid filter (need literal on rhs)");
        ...
        final Optional<Map.Entry<String, Path>> optionalPathEntry = swagger.getPaths().entrySet().stream().filter(entry -> {
